...
metadata$location <- fct_relevel(metadata$location, "Nasal Swab", "Tracheal Swab", "Rectal Swab", "Stool") #Refactor locations
metadata$time <- fct_relevel(metadata$time, "-7", "3", "5", "7", "10", "14") #Refactor time into chronological order
...
#Keeps colors consistant across graphs
...
#Keeps theme constant across graphs
...
#Observed Features (Supplemental Figure 2A)
...
p_observed_features_location <- ggplot(data = observed_features, aes(x = location, y = observed_features)) +
  geom_boxplot() +
  geom_point() +
  geom_pwc(stat = "pwc", method = "dunn_test",label = "{p.adj.signif}", vjust = 0.6, hide.ns = TRUE, p.adjust.method = "BH",  #Statistical test for observed features
...
#Relative abundance of controls (Supplemental Figure 2B)
taxa_long_genus_control <-
  ...
  filter(location == 'Negative Control') %>% #Removes the Postive controls
  group_by(SampleID) %>%
  mutate(Total_Abundance = sum(Abundance)) %>% #Calculates the total abundance for each sample
  ungroup()  %>%
  mutate(Relative_Abundance = Abundance/Total_Abundance) %>% #Calculates relative abundance
  mutate(LTC = if_else(Relative_Abundance > 0.05, Taxon, "Other")) %>% #Taxa that have an abundance of less than 5% are designated as other
...
#Beta Diversity (Supplemental Figure 2C)
...
#12 Relative abundance phylum level (Supplemental Figure 10)
...
# Differential Abundance (Supplemental Figure 11)
